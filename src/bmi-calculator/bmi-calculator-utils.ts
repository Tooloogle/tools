...
const validateHeightRange = (value: number, unit: string): BMIValidationResult => {
    ...
    if (value < minHeight || value > maxHeight) {
        return {
            isValid: false,
            error: `Height must be between ${minHeight}-${maxHeight} ${unit === 'metric' ? 'cm' : 'ft'}`
        };
    }
...
const validateWeightRange = (value: number, unit: string): BMIValidationResult => {
    ...
    if (value < minWeight || value > maxWeight) {
        return {
            isValid: false,
            error: `Weight must be between ${minWeight}-${maxWeight} ${unit === 'metric' ? 'kg' : 'lbs'}`
        };
    }
...
export const validateInput = (value: number, type: 'height' | 'weight', unit: string): BMIValidationResult => {
...
export const calculateBMIValue = (height: number, weight: number, unit: string): number => {
