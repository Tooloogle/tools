...
import { calculateMD5 } from '../_libs/md5-hash-generator/md5.js';
...
export class Md5HashGenerator extends WebComponentBase<IConfigBase> {
    ...
    private validateInput(): boolean {
        ...
        if (this.input.length > this.MAX_INPUT_SIZE) {
            this.error = `Input is too large (max ${this.MAX_INPUT_SIZE / 1024}KB allowed)`;
            return false;
        }
    ...
    async generateHash() {
        try {
            ...
            // Calculate in next tick to allow UI to update
            await new Promise(requestAnimationFrame);
            this.hash = calculateMD5(this.input);
        } catch (err) {
    ...
    override render() {
        ...
        <div>
            ...
            ${this.error ? html`<div class="text-red-500 mb-2">${this.error}</div>` : ''}
            ...
            <div class="button-group">
                ...
                <button class="btn btn-red" @click=${this.clearAll} ?disabled=${this.isGenerating}>
                    Clear All
                </button>
            </div>
