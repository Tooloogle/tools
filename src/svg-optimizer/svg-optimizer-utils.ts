...
export class SvgOptimizerUtils {
  ...
  static convertStylesToAttributes(
    optimized: string,
    options: OptimizationOptions
  ): string {
  ...
  static removeUselessAttributes(
    optimized: string,
    options: OptimizationOptions
  ): string {
  ...
  static removeUnusedNamespaces(
    ...
    namespaces.forEach(([fullMatch, prefix]) => {
      const isUsed = new RegExp(`${prefix}:`).test(
  ...
  static async processOptimization(
    svgContent: string,
    options: OptimizationOptions
  ): Promise<string> {
...
export class FileUtils {
  ...
  static async copyToClipboard(text: string): Promise<void> {
    try {
      ...
      // ⚠️ Fallback for older browsers that don't support navigator.clipboard
      // document.execCommand('copy') is deprecated but kept here intentionally for backward compatibility.
  ...
  static validateSvgContent(content: string): {
    isValid: boolean;
    error?: string;
  } {
