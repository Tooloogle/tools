...
export class MorseCodeTranslator extends WebComponentBase<IConfigBase> {
    static override styles = [WebComponentBase.styles, inputStyles, morseCodeTranslatorStyles];
    ...
    private process() {
        ...
        if (this.inputText.match(/^[.\-\s/]+$/)) {
            // Decode morse
            this.outputText = this.inputText.split(' ').map(code => reverseMorse[code] || code).join('');
        } else {
            // Encode to morse
            this.outputText = this.inputText.toUpperCase().split('').map(c => morseMap[c] || c).join(' ');
        }
    }
