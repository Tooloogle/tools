...
export class DataUriGenerator extends WebComponentBase<IConfigBase> {
    static override styles = [WebComponentBase.styles, inputStyles, dataUriGeneratorStyles];
    ...
    private process() {
        ...
        try {
            ...
            this.outputText = 'Error: Unable to encode text. Please check your input.';
            console.error(err);
        }
    }
        ...
        return html`
            <div class="space-y-4">
                <div>
                    ...
                    <select class="form-input w-full" .value=${this.mimeType} @change=${this.handleMimeTypeChange}>
                ...
                <div>
                    <label class="block mb-2">
                        <input type="checkbox" .checked=${this.useBase64} @change=${this.handleBase64Change} />
                        <span class="ml-2">Use Base64 encoding</span>
                    </label>
                </div>
                <div>
                    <label class="block mb-2 font-semibold">Input Text:</label>
                    <textarea class="form-input w-full h-32" placeholder="Enter text to convert to Data URI..."
                        .value=${this.inputText} @input=${this.handleInput}></textarea>
                </div>
                <div>
                    <label class="block mb-2 font-semibold">Data URI Output:</label>
                    <textarea class="form-input w-full h-32 font-mono text-sm" readonly .value=${this.outputText}></textarea>
                    ${this.outputText && !this.outputText.startsWith('Error:') ? html`<t-copy-button .text=${this.outputText}></t-copy-button>` : ''}
                </div>
                ...
                <div class="text-sm text-gray-600">
                    <p><strong>Note:</strong> Data URIs allow you to embed data directly in HTML/CSS.</p>
