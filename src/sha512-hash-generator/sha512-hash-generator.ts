...
export class Sha512HashGenerator extends WebComponentBase {
  static override styles = [
    WebComponentBase.styles,    sha512HashGeneratorStyles];
  ...
  private async process() {
    ...
    if (!isBrowser() || !window.crypto || !window.crypto.subtle) {
      this.error =
        'SHA-512 hashing requires a modern browser with Web Crypto API support';
      this.outputText = '';
      return;
    }
