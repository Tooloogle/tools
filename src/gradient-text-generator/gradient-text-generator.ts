...
export class GradientTextGenerator extends WebComponentBase<IConfigBase> {
    static override styles = [WebComponentBase.styles, inputStyles, gradientTextGeneratorStyles];
        ...
        return html`
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" class="form-input w-full" placeholder="Enter text..."
                        .value=${this.inputText} @input=${this.handleInput} />
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-2 font-semibold">Color 1:</label>
                        <div class="flex gap-2">
                            <input type="color" .value=${this.color1} @input=${this.handleColor1Change} class="h-10 w-16" />
                            <input type="text" class="form-input" .value=${this.color1} @input=${this.handleColor1Change} />
                        </div>
                    </div>
                    <div>
                        <label class="block mb-2 font-semibold">Color 2:</label>
                        <div class="flex gap-2">
                            <input type="color" .value=${this.color2} @input=${this.handleColor2Change} class="h-10 w-16" />
                            <input type="text" class="form-input" .value=${this.color2} @input=${this.handleColor2Change} />
                        </div>
                    </div>
                </div>
                <div>
                    <label class="block mb-2 font-semibold">Gradient Type:</label>
                    <select class="form-input w-full" .value=${this.gradientType} @change=${this.handleGradientTypeChange}>
                ...
                ${this.gradientType === 'linear' ? html`
                    <div>
                        <label class="block mb-2 font-semibold">Angle: ${this.angle}°</label>
                        <input type="range" min="0" max="360" class="w-full" .value=${String(this.angle)} @input=${this.handleAngleChange} />
                    </div>
                ` : ''}
                <div class="p-4 bg-white border rounded text-center">
                    <p class="font-bold mb-2">Preview:</p>
                    <div style="background: ${gradientStyle}; -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; font-size: 48px; font-weight: bold;">
                        ${this.inputText}
                    </div>
                </div>
                <div>
                    <label class="block mb-2 font-semibold">CSS Code:</label>
                    <textarea class="form-input w-full h-40 font-mono text-sm" readonly .value=${cssCode}></textarea>
                    <t-copy-button .text=${cssCode}></t-copy-button>
                </div>
            </div>
        `;
    }
