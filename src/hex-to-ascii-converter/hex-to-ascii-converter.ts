...
export class HexToAsciiConverter extends WebComponentBase<IConfigBase> {
    static override styles = [WebComponentBase.styles, inputStyles, hexToAsciiConverterStyles];
    ...
    private convert() {
        try {
            this.outputText = this.inputText.split(/\s+/).map(h => String.fromCharCode(parseInt(h, 16))).join('');
        } catch (e) {
