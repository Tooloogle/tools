...
export class Sha256HashGenerator extends WebComponentBase<IConfigBase> {
    static override styles = [WebComponentBase.styles, inputStyles, sha256HashGeneratorStyles];
    ...
    private async process() {
        ...
        if (!isBrowser() || !window.crypto || !window.crypto.subtle) {
            this.error = 'SHA-256 hashing requires a modern browser with Web Crypto API support';
            this.outputText = '';
            return;
        }
