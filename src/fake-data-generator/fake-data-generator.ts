...
export class FakeDataGenerator extends WebComponentBase<IConfigBase> {
    ...
    private firstNames = ['John', 'Jane', 'Michael', 'Sarah', 'David', 'Emily', 'Chris', 'Emma', 'James', 'Lisa'];
    private lastNames = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis', 'Wilson', 'Moore'];
    private domains = ['gmail.com', 'yahoo.com', 'outlook.com', 'example.com', 'test.com'];
    private streets = ['Main St', 'Oak Ave', 'Park Rd', 'Elm St', 'Maple Dr', 'Lake View', 'Hill St'];
    private cities = ['New York', 'Los Angeles', 'Chicago', 'Houston', 'Phoenix', 'Philadelphia', 'San Antonio'];
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select class="form-input w-full" .value=${this.dataType}
                            @change=${(e: Event) => { this.dataType = (e.target as HTMLSelectElement).value; this.generate(); }}>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            @input=${(e: Event) => { this.count = Number((e.target as HTMLInputElement).value); this.generate(); }}
                        />
                    </div>
                </div>
                <div>
                    ...
                    <textarea
                        class="form-input w-full h-64 font-mono text-sm"
                        readonly
                        .value=${this.result}
                    ></textarea>
                    <t-copy-button .text=${this.result}></t-copy-button>
                </div>
                <button class="btn btn-blue" @click=${this.generate}>Generate New</button>
            </div>
        `;
    }
}
